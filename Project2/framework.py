...
class Linear(Module) :
    ...
    def __init__(self, d_in, d_out, randType = 'normal', mean = 0., sigma = 1.):
        ...
        # Define weights
        switch_w = {
            'normal' : empty((d_out, d_in)).normal_(mean,sigma).mul_(1/m.sqrt(d_in)),
            'uniform' : empty((d_out, d_in)).uniform_(-1/m.sqrt(d_in),1/m.sqrt(d_in))
        }
        self.w = switch_w.get(randType)

        if (self.w is None) : raise TypeError("Error : wrong randType value for init of fully connected layer")
    ...
    def backward_pass(self, dl_ds):
        ...
        print('ds',dl_ds.size())
        print('db',self.dl_db.size())
        print('dw',self.dl_dw.size())
...
class lossMSE(Module):
    ...
    def backward_pass(self, input, target):
        a=2 * (input - target)/input.size(1)
        print('outputloss',a.size())
