...
number_run=10
#Model allows to chose wich model we want to use for the training and testing
MODEL=1 #   1 : complex model 
        #   2 : simple model
        #   3 : resnet model
...
for k in range(number_run):  # run 10 time our algorithm to get a better approximation of the performance of our model 
    ...
    if MODEL==1:
        ...
        ut.train_model(model, train_input, train_target, train_classes, test_input, test_target, test_classes, mini_batch_size, error_train_epoch, error_test_epoch, aux_loss=True, weight=2, eta=1, choose_optimizer=2,plot=PLOT)  
