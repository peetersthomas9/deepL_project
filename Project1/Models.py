...
mean test error :   22,41 %
standard deviation : 1.3972% 
computation time per training : 1.57 s
...
"""
This model is our complex model with two convolution and 3 or 4 linear layers depending on which auxiliary loss method we want to use. 
...
mean test error :   2.26 %
standard deviation : 0.5542% 
computation time per training : 7.01s
"""
class Netfinal(nn.Module):
    ...
    def forward(self, x):
        # x is [N,2,14,14] and we want to get x1 = [N,1,14,14] and x2= [N,1,14,14] so that we classify one image at a time and not
        ...
        # concatenate x1, x2 (earlier output) or x1class,x2class (final output) depending on the auxialiary loss method choosen
        ...
        # the output will be x1class [N,10] and x2class [N,10] corresponding to the probability of the number on each images
...
mean test error :   17.76 %
standard deviation : 1.6392% 
computation time per training : 302 s
"""
