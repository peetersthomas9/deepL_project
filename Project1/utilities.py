#Utilities 
"""
We have in this file all the functions that we will use to train our model and to test the performance of our model 
"""
...
input : 
    ...
    test_input, test_target, test_classes: the data and the labels to test our model
...
def train_model(model, train_input, train_target, train_classes, test_input, test_target, test_classes, mini_batch_size, error_train, error_test, nb_epochs = 25,aux_loss=True, weight=3, eta=1.1, choose_optimizer=2,plot=False):
    ...
    if model.__class__.__name__=='Netfinal': # different numbers of output depending on the model 
        ...
        for k in range(data_target.size(0)):
            if data_target[k] != predicted_target[k]:  
                nb_data_errors_target = nb_data_errors_target + 1           # number of error for the output
